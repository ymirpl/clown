...
def index(request):
    ...
    recent_tuits = InfinitePaginator(Tuit.objects.select_related('user').all().order_by("-added"), 500).page(page)
...
def index_wo_user(request):
    page = request.GET.get('page', 1)
    recent_tuits = InfinitePaginator(Tuit.objects.all().order_by("-added"), 500).page(page)
